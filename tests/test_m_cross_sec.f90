program test_m_cross_sec
   ...
   !call CS_add_from_file(first_in, gas_1, 1.0_dp, 1.0e5_dp, cross_secs, opt_out_of_bounds_upper=CS_OutOfBounds_Throw)

   print *, "TEST 1: Out of bounds upper: CS_OutOfBounds_Zero: If input data energy does not reach required energy add a 0 cs line."
   call CS_add_from_file(first_in, gas_1, 1.0_dp, TEST_1_req_energy, TEST_1_cross_secs, opt_out_of_bounds_upper=CS_OutOfBounds_Zero)
   ...
   print *, "TEST 2: Out of bounds upper: CS_OutOfBounds_Linear: If input data energy does not reach required energy ", &
            "linearly interpolate to it."
   call CS_add_from_file(first_in, gas_1, 1.0_dp, TEST_2_req_energy, TEST_2_cross_secs, &
                         opt_out_of_bounds_upper=CS_OutOfBounds_Linear)
   ...
   print *, "TEST 3: Out of bounds upper: CS_OutOfBounds_Linear: If input data energy does not reach required energy ", &
            "linearly interpolate to it BUT if interpolated value is negative warn user and set cs to 0."
   call CS_add_from_file(first_in, gas_1, 1.0_dp, TEST_3_req_energy, TEST_3_cross_secs, &
                         opt_out_of_bounds_upper=CS_OutOfBounds_Linear)
   ...
   print *, "TEST 4: Out of bounds lower: CS_OutOfBounds_Constant: Keep the first (energy, cs) pair as it is."
   call CS_add_from_file(first_in, gas_1, 1.0_dp, 100.0_dp, TEST_4_cross_secs, &
                         opt_out_of_bounds_lower=CS_OutOfBounds_Constant)
   ...
   print *, "TEST 5: Out of bounds lower: CS_OutOfBounds_Zero: Add an extra line at the start of the input data ", &
   ...
   call CS_add_from_file(first_in, gas_1, 1.0_dp, 100.0_dp, TEST_5_cross_secs, &
                         opt_out_of_bounds_lower=CS_OutOfBounds_Zero)
   ...
   print *, "TEST 6: Out of bounds lower: CS_OutOfBounds_Zero: SHOULD NOT Add an extra line at the start of the input data ", &
            "with energy 1% less than first point in the input data and the cross section 0"
